...
// Locale configuration for AI Content Generation - Only French, Dutch, and English
...
export default function AIContentLocaleSelect({ register, defaultValue, className = "" }) {
  ...
  const options = aiContentLocales.map((locale) => ({
    value: locale.value,
    label: locale.label,
  ...
  const customTrigger = (isOpen) => {
    const selectedLocaleOption = aiContentLocales.find(locale => locale.value === selectedLocale);
    return (
  ...
  const renderOption = (option, isSelected) => {
    const localeData = aiContentLocales.find(loc => loc.value === option.value);
    return (
