...
// Supported preferred languages (matching backend enum)
const preferredLanguages = ['en', 'fr', 'nl'];

const preferredLanguageNames = {
  en: 'English',
  fr: 'Français',
  nl: 'Nederlands',
};

export default function PreferredLanguageSelect({ register, defaultValue, className = "" }) {
  const { locale: currentLocale } = useI18n();
  // Map current locale to preferred language (en, fr, nl only)
  ...
  const defaultLocale = defaultValue || mapToPreferredLocale(currentLocale);
  ...
  const options = preferredLanguages.map((locale) => ({
    value: locale,
    label: preferredLanguageNames[locale],
    icon: <HiGlobeAlt className="h-5 w-5 text-gray-400" />,
  }));

  const selectedOption = options.find((opt) => opt.value === selectedLocale);

  // Custom trigger with icon on the left
    ...
    <div className="relative w-full">
      ...
      >
        <span className={selectedOption ? "text-gray-900" : "text-gray-500"}>
          {selectedOption ? selectedOption.label : "Select preferred language"}
        </span>
        <svg
